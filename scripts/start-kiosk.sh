...
if command -v xset >/dev/null; then xset s off || true; xset -dpms || true; xset s noblank || true; else log "xset no disponible"; fi
if command -v openbox >/dev/null; then
  [[ "${BASCULA_KIOSK_REPLACE_WM:-0}" == "1" ]] && { log "Ejecutando openbox --replace"; openbox --replace &; } || { log "Lanzando openbox"; openbox &; }
else log "openbox no encontrado"; fi
if UNCLUTTER_BIN="$(command -v unclutter 2>/dev/null || true)"; [[ -n "${UNCLUTTER_BIN}" ]]; then "${UNCLUTTER_BIN}" -idle 0.5 -root & UNCLUTTER_PID=$!; log "unclutter iniciado (pid=${UNCLUTTER_PID})"; else log "unclutter no instalado"; fi
sleep 1
TARGET_URL="http://localhost/"
for attempt in $(seq 1 20); do
  curl -sf http://127.0.0.1:8081/api/health >/dev/null && break || sleep 1
done
if curl -sf http://127.0.0.1:8081/api/health >/dev/null; then
  log "Backend disponible, abriendo ${TARGET_URL}"
else
  TARGET_URL="http://localhost/config"; log "Backend no respondió a tiempo, abriendo ${TARGET_URL}"
fi
