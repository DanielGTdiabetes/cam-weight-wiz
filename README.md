...
| POST | `/api/camera/capture-to-file` | Captura un JPEG one-shot (`RGB888`) y lo guarda en `/tmp/camera-capture.jpg`, devolviendo `{ ok, path, size }`. |
...
ls -lh /tmp/camera-capture.jpg
file /tmp/camera-capture.jpg
```

En la mini-web, el botón “Activar cámara” realiza el `POST` anterior y muestra `/tmp/camera-capture.jpg?t=<epoch_ms>` para forzar un cache-buster en cada intento. Así se pueden repetir capturas sucesivas sin que el navegador reutilice la imagen previa.
