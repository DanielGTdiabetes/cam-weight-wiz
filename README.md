...
| GET | `/api/camera/last` | Devuelve el último JPEG guardado en `/tmp/camera-capture.jpg` sin cachearlo. |
...
En la mini-web, el botón “Activar cámara” realiza el `POST` anterior y muestra `/api/camera/last?ts=<epoch_ms>` para forzar un cache-buster en cada intento. Así se pueden repetir capturas sucesivas sin que el navegador reutilice la imagen previa.
